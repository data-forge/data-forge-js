...
describe('skip-while iterator', function () {
	...
	it('result is undefined before moving to first element', function () {
		...
		var testObject = new SkipWhileIterator(makeArrayIterable([1]), function () { return false; });
		expect(testObject.getCurrent()).to.be.undefined;
	});

	it('cannot move next for empty iterator with no skip', function () {

		var testObject = new SkipWhileIterator(makeArrayIterable([]), function () { return false; });
		expect(testObject.moveNext()).to.eql(false);
	});

	it('cannot move next for empty iterator with skip', function () {

		var testObject = new SkipWhileIterator(makeArrayIterable([]), function () { return true; });
		expect(testObject.moveNext()).to.eql(false);
	});
	...
	it('cannot move next if skipping everything', function () {
		...
		var testObject = new SkipWhileIterator(makeArrayIterable([1]), function () { return true; });
		expect(testObject.moveNext()).to.eql(false);
	});

	it('can skip based on values from child iterator', function () {

		var testObject = new SkipWhileIterator(makeArrayIterable([true, true, false, true]), function (value) { return value; });
		expect(testObject.moveNext()).to.eql(true);
		expect(testObject.moveNext()).to.eql(true);
		expect(testObject.moveNext()).to.eql(false);
	});
	...
	it('can skip a certain number of values', function () {
		...
		var testObject = new SkipWhileIterator(makeArrayIterable([1, 2, 3, 4]), function (value) { return value < 3; });
		expect(testObject.moveNext()).to.eql(true);
